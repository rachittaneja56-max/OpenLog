...
export async function createEntryController(
  ...
  try {
    ...
    const entry = await createEntry(route.slug, input, request.cookies ?? {});
...
export async function updateEntryController(
  ...
  try {
    ...
    const entry = await updateEntry(route.slug, route.entryId, input, request.cookies ?? {});
...
export async function deleteEntryController(
  ...
  try {
    ...
    await deleteEntry(route.slug, route.entryId, request.cookies ?? {});
